    ...
    <div className="min-h-screen p-6">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-400">Review and manage NGO funding requests</p>
        </div>

        {/* Stats */}
        <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            { label: 'Total Requests', value: stats.total, icon: DollarSign, color: 'blue' },
            ...
            <div key={idx} className="rounded-xl border border-gray-700 bg-gray-800/50 p-5">
        ...
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-lg border border-gray-700 bg-gray-800 py-2 pl-10 pr-4 text-white placeholder-gray-400 focus:border-red-500 focus:outline-none sm:w-64"
            />
          </div>
        </div>
        ...
        {filteredRequests.length === 0 ? (
          <div className="rounded-2xl border border-gray-700 bg-gray-800/50 py-20 text-center">
            ...
            <h3 className="mb-2 text-lg font-semibold text-white">No requests found</h3>
            <p className="text-gray-400">No funding requests match your current filters</p>
          </div>
        ) : (
              ...
              >
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  <div className="flex-1">
                    ...
                    <p className="mb-4 text-gray-400">{request.description.substring(0, 200)}...</p>

                    <div className="flex flex-wrap gap-4 text-sm">
                      <div className="flex items-center gap-2">
                        <DollarSign className="h-4 w-4 text-green-400" />
                        <span className="text-gray-300">₹{request.requestedAmount.toLocaleString()}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Package className="h-4 w-4 text-blue-400" />
                        <span className="text-gray-300 capitalize">{request.purpose}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Users className="h-4 w-4 text-purple-400" />
                        <span className="text-gray-300">{request.beneficiaryCount} beneficiaries</span>
                      </div>
                      <div className="flex items-center gap-2">
                      ...
                      <>
                        <button
                          onClick={() => {
                            ...
                            setActionData({ ...actionData, approvedAmount: request.requestedAmount.toString() });
      ...
      {showDetailModal && selectedRequest && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-2xl border border-gray-700 bg-gray-900">
            <div className="sticky top-0 z-10 border-b border-gray-700 bg-gray-800/95 backdrop-blur-sm px-6 py-4">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-bold text-white">{selectedRequest.title}</h2>
                  <div className="mt-1">{getStatusBadge(selectedRequest.adminStatus)}</div>
                </div>
                <button
                  onClick={() => {
            ...
            <div className="space-y-6 p-6">
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="rounded-lg border border-gray-700 bg-gray-800/50 p-4">
                  <div className="mb-1 text-sm text-gray-400">Requested Amount</div>
                ...
                <div className="rounded-lg border border-gray-700 bg-gray-800/50 p-4">
                  ...
                  <p className="text-xl font-semibold capitalize text-white">{selectedRequest.purpose}</p>
                </div>
              </div>

              <div className="rounded-lg border border-gray-700 bg-gray-800/50 p-4">
                <h3 className="mb-2 text-sm font-medium text-gray-400">Description</h3>
                <p className="whitespace-pre-wrap text-gray-300">{selectedRequest.description}</p>
              </div>

              {selectedRequest.budgetBreakdown && selectedRequest.budgetBreakdown.length > 0 && (
                <div className="rounded-lg border border-gray-700 bg-gray-800/50 p-4">
                  <h3 className="mb-3 text-sm font-medium text-gray-400">Budget Breakdown</h3>
                  <div className="space-y-2">
                    {selectedRequest.budgetBreakdown.map((item, idx) => (
                      <div key={idx} className="flex items-center justify-between rounded-lg border border-gray-700 bg-gray-900/50 p-3">
                        <div>
                          <p className="font-medium text-white">{item.item}</p>
                          {item.description && <p className="text-sm text-gray-400">{item.description}</p>}
                        </div>
                        <p className="font-semibold text-green-400">₹{Number(item.amount || 0).toLocaleString()}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {/* Submitted On */}
              <div className="rounded-lg border border-gray-700 bg-gray-800/50 p-4">
                <h3 className="mb-1 text-sm font-medium text-gray-400">Submitted On</h3>
              ...
              {/* Approved Amount */}
              {selectedRequest.adminStatus === 'approved' && selectedRequest.approvedAmount > 0 && (
                <div className="rounded-lg border border-green-500/30 bg-green-500/10 p-6">
                  <div className="mb-3 flex items-center gap-2">
                    <CheckCircle className="h-5 w-5 text-green-400" />
                    <h3 className="text-lg font-semibold text-green-400">Approved Amount</h3>
                  </div>
                  ...
                  {selectedRequest.adminNotes && (
                    <div className="mb-3 rounded-lg border border-green-500/20 bg-green-500/5 p-3">
                      <p className="mb-1 text-xs font-medium text-green-400">Admin Notes:</p>
                      <p className="text-sm text-gray-300">{selectedRequest.adminNotes}</p>
                    </div>
                  )}
                  
                  {selectedRequest.adminReviewedAt && (
                    <p className="text-xs text-green-300">
                      Approved on {new Date(selectedRequest.adminReviewedAt).toLocaleString('en-IN', {
                        day: 'numeric',
                        month: 'short',
                        year: 'numeric',
                        hour: '2-digit',
              ...
              {/* Rejection Reason */}
              {selectedRequest.adminStatus === 'rejected' && selectedRequest.rejectionReason && (
                <div className="rounded-lg border border-red-500/30 bg-red-500/10 p-6">
                  ...
                  {selectedRequest.adminReviewedAt && (
                    <p className="mt-3 text-xs text-red-300">
                      Rejected on {new Date(selectedRequest.adminReviewedAt).toLocaleString('en-IN', {
                        day: 'numeric',
                        month: 'short',
                        year: 'numeric',
                        hour: '2-digit',
